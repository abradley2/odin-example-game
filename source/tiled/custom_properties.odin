...
Bounce_Crawler_Spawn :: struct {}

Spike_Crawler_Spawn :: struct {}
...
custom_property_from_raw :: proc(
	...
	if raw_custom_property.name == "collision_box" {
		push_context(parse_context, "collision_box")
		switch val in raw_custom_property.value {
		case bool:
			if val {
	...
	if raw_custom_property.name == "spawn_for" {
		...
		case string:
			...
			case "bounce_crawler":
				custom_property = Bounce_Crawler_Spawn{}
			case "spike_crawler":
				custom_property = Spike_Crawler_Spawn{}
			case:
				return
			}
		case i64, f64, bool:
